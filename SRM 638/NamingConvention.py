...
def do_test(variableName, __expected):
    startTime = time.time()
...
def run_tests():
    ...
    with open("NamingConvention.sample", "r") as f:
        while True:
            label = f.readline()
            if not label.startswith("--"): break

            variableName = f.readline().rstrip()
            f.readline()
            ...
            passed += do_test(variableName, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1479889806
    PT, TT = (T / 60.0, 75.0)
